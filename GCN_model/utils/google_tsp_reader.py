...
class GoogleTSPReader(object):
    ...
    def __init__(self, num_nodes, num_neighbors, batch_size, filepath, split_details=None, shuffleDataset=True):
        """
        Args:
        ...
        if (split_details is not None):
            print("split ", split_details)
            split_content_percentage = split_details['split_percentage']
            split_id = split_details['split_id'] # support 0, query 1


            file_data = open(filepath, "r").readlines()
            num_samples = len(file_data)

            samples_in_this_split = int(num_samples*split_content_percentage/100)
            
            print(" samples_in_this_split ", samples_in_this_split)


            if(split_id==0):
                self.filedata = file_data[0:samples_in_this_split]  # Always shuffle upon reading data


            if (split_id == 1):
                from_index = int((100-split_content_percentage)*num_samples/100)
                to_index = int((100-split_content_percentage)*num_samples/100) + samples_in_this_split
                print(from_index, to_index)
                self.filedata = file_data[from_index:to_index]
            if(shuffleDataset==True):
