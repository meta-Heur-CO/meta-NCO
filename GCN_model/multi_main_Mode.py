...
if notebook_mode==False:
    ...
    parser.add_argument('-c','--config', type=str, default="configs/gmm_multi/multi_config.json")
...
for task in tasks_list:
    config_path_tasks = "configs/mode_multi/tsp30_mode{}".format(task)+'.json'#args.config
...
def test_ft(net_meta, config_meta, config_task, master_bar, mode='test', num_fine_tune_steps=1000):
    # Set evaluation mode
    ...
    net_task = copy.copy(net_meta)#net_task
    net_task.train()
    ...
    batches_per_epoch = num_fine_tune_steps#10000
    print("batches per epoch", batches_per_epoch)
    # Convert dataset to iterable
    ...
    for epoch in epoch_bar:
        ...
        for meta_steps in range(0,num_meta_steps ):
            ...
            for task in tasks_list:
                ...
                #X from inside train epoch since tsp read is slow
                ...
                train_time, train_loss, train_err_edges, train_err_tour, train_err_tsp, train_pred_tour_len, train_gt_tour_len = train_one_step(net_meta,
                 config_task, config_meta, epoch_bar, dict_task_GTSP_reader[task])
