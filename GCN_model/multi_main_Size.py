...
if notebook_mode==False:
    ...
    parser.add_argument('-c','--config', type=str, default="configs/gsize_multi/multi_config.json")
...
for task in tasks_list:
    config_path_tasks = "configs/size_multi/"+'tsp'+str(task)+'.json'
...
def train_one_step(net_meta, config_task, config_meta, master_bar, dataset_loaded):
    # Set training mode
    ...
    dataset_support = dataset_loaded
...
def test_ft(net_meta, config_meta, config_task, master_bar, mode='test', num_fine_tune_steps=1050):
    ...
    print(" BATCHES PER EPOCH FINE_TUNING", batches_per_epoch)
    ...
    for batch_num in progress_bar(range(batches_per_epoch), parent=master_bar):
        ...
        if(batch_num%50==0 and mode=='test'):
            ...
            print(" at step ", batch_num, " opt gap dataset is  ", opt_gap_dataset)
    ...
    for epoch in epoch_bar:
        ...
        for meta_steps in range(0,num_meta_steps ):
            ...
            for task in tasks_list:
                ...
                # from inside train epoch since tsp read is slow
                num_nodes_task = config_task.num_nodes
                num_neighbors_task = config_task.num_neighbors
                batch_size = config_meta.batch_size
                train_task_filepath = config_task.train_filepath
            
                # Load TSP data
                if(meta_steps%50==0):
                    print("load again with reshuffle")
                    dict_task_GTSP_reader[task] = GoogleTSPReader(num_nodes_task, num_neighbors_task, batch_size, train_task_filepath)


                writer.add_scalar('learning_rate', learning_rate, epoch)

                # Train
                train_time, train_loss, train_err_edges, train_err_tour, train_err_tsp, train_pred_tour_len, train_gt_tour_len = train_one_step(net_meta,config_task,                 config_meta, epoch_bar, dict_task_GTSP_reader[task])
        ...
        if epoch % val_every == 0 or epoch == max_epochs - 1:
            ...
            for task in tasks_list:
                ...
                val_time, val_loss, val_err_edges, val_err_tour, val_err_tsp, val_pred_tour_len, val_gt_tour_len, opt_gap_dataset = test(
                    net_meta, config_task, epoch_bar, mode='val')
        ...
        if epoch % test_every == 0 or epoch == max_epochs - 1:
            ...
            # Test
                test_time, test_loss, test_err_edges, test_err_tour, test_err_tsp, test_pred_tour_len, test_gt_tour_len, opt_gap_dataset = test(
                    net_meta, config_task, epoch_bar, mode='test')
    ...
    for task in tasks_lists_test:
        ...
        for num_fsteps in number_of_fst_steps_list:
            ...
            for epoch in epoch_bar:
                ...
                t=time.time()
                val_time, val_loss, val_err_edges, val_err_tour, val_err_tsp, val_pred_tour_len, val_gt_tour_len, opt_gap_dataset = test_ft(
